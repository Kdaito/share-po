...
export type Props = {
  onClickConfirm: (value: Player) => void;
  defaultValue?: Player;
};

const AddPlayer: React.FC<Props> = ({ onClickConfirm, defaultValue }) => {
      ...
      <Modal isOpen={isOpen} onClose={closeModal}>
        <Box sx={{ width: "500px" }}>
          ...
          <Typography variant="h5" component="p" align="left">
            背番号 : {newPlayerData?.number} / ポジション :{" "}
            {getPositionText(newPlayerData?.position)}
          </Typography>
          <Button
            fullWidth
            variant="contained"
